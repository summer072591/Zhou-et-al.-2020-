...
WrapFull <- function(s){
   ...
   beta.original <- c(sample(c(-1, 1), size = 50, replace = TRUE, prob = c(0.5, 0.5)), rep(0, 450))
   # beta.original <- c(rnorm(5), rep(0, 495))

    beta.original <- sample(beta.original, size = 500, replace = FALSE)
 
    # beta.original <- sample(c(-1, 1, 0), size = 500, replace = TRUE, prob = c(0.005, 0.005, 0.99))
...
#  for(k in quantiles){
      ...
      #  test_coef_dist(seed, err.dist = "mixnorm", omega = sparse.omega, K = k, n = n, p = p, alpha.range = alpha.range, 
      #                                            sparse.omega = sparse.omega, A = A, x = x,tau = tau)

     #   test_coef_dist(seed, err.dist = "t2", omega = sparse.omega, K= k, n = n, p = p, alpha.range = alpha.range, 
     #                                             sparse.omega = sparse.omega, A = A, x = x, tau = tau)
     #   }
